...
modules/                               # All .nix files here, auto-imported recursively
  ...
  programs/<name>.nix                   # Single-file program feature
  programs/<name>/<name>.nix           # Multi-file program feature (has siblings)
  services/<name>.nix                  # Single-file service feature
  services/<name>/<name>.nix           # Multi-file service feature (has siblings)
...
10. **Prefer flat files over single-file directories.** Use `modules/<category>/<name>.nix` for features that are a single file. Only promote to a directory (`<name>/<name>.nix`) when the feature needs sibling files (extra configs, patches, assets). A directory containing one `.nix` file and nothing else is unnecessary ceremony.

11. **Prefer nixpkgs packages over homebrew.** Homebrew should only be a last resort for packages genuinely unavailable in nixpkgs.

12. **Aspect names must be valid Nix identifiers.** Avoid names starting with a digit.
...
3. Create the feature file: `modules/<category>/<name>.nix` for single-file features, or `modules/<category>/<name>/<name>.nix` if it will have sibling files
